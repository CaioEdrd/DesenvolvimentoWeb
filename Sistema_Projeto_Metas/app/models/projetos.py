...
class Projeto(db.Model): #cria a tabela "Projeto"
    ...
    data_criacao = db.Column(db.DateTime, default=datetime.now().strftime('%d/%m/%Y %H:%M:%S')) #utiliza da biblioteca date time para puxar a data e hora de criação

    metas = db.relationship("Meta", backref="projeto", lazy=True)